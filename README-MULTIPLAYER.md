...
5. **Environment** sekmesinden şu değişkenleri ekle:
   - `ANTHROPIC_API_KEY` — Claude API anahtarın (console.anthropic.com'dan alınır). **Bu olmadan AI panel çalışmaz** — anahtar yoksa panel, yazdığın metni yorumlamadan ham ilan olarak ekler (sistem çökmez ama AI devre dışı kalır).
   ...
   - `ANTHROPIC_MODEL` (opsiyonel) — varsayılan `claude-sonnet-5`. Güncel model isimlerini docs.claude.com'dan kontrol edebilirsin.
...
- Sunucu bu metni Claude API'ye gönderir, Claude onu oyuna uygulanabilecek bir eyleme çevirir:
...
- `server.js`: ortak dünya, gerçek zamanlı bağlantı, Claude API entegrasyonu, kalıcılık
